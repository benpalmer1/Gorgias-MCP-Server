...
export function registerVoiceCallTools(server: McpServer, client: GorgiasClient) {
  ...
  server.registerTool("gorgias_list_voice_calls", {
    ...
    inputSchema: {
      cursor: cursorSchema.optional().describe("Cursor value for pagination (use next_cursor or prev_cursor from a previous response)"),
      ...
      ticket_id: idSchema.optional().describe("Filter voice calls belonging to a specific ticket"),
    },
  ...
  server.registerTool("gorgias_get_voice_call", {
    ...
    inputSchema: {
      id: idSchema.describe("The ID of the voice call to retrieve"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  ...
  server.registerTool("gorgias_list_voice_call_events", {
    ...
    inputSchema: {
      cursor: cursorSchema.optional().describe("Pagination cursor for fetching the next or previous page"),
      limit: z.number().int().min(1).max(100).optional().describe("Maximum number of records to return per page (1-100, default 30)"),
      call_id: idSchema.optional().describe("Filter events to those belonging to a specific voice call"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  }, safeHandler(async (args) => {
  ...
  server.registerTool("gorgias_get_voice_call_event", {
    ...
    inputSchema: {
      id: idSchema.describe("The unique identifier of the voice call event to retrieve"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  }, safeHandler(async ({ id }) => {
  ...
  server.registerTool("gorgias_list_voice_call_recordings", {
    ...
    inputSchema: {
      cursor: cursorSchema.optional().describe("Pagination cursor for fetching the next or previous page"),
      limit: z.number().int().min(1).max(100).optional().describe("Maximum number of records to return per page (1-100, default 30)"),
      call_id: idSchema.optional().describe("Filter recordings to those belonging to a specific voice call"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  }, safeHandler(async (args) => {
  ...
  server.registerTool("gorgias_get_voice_call_recording", {
    ...
    inputSchema: {
      id: idSchema.describe("The unique identifier of the voice call recording to retrieve"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  }, safeHandler(async ({ id }) => {
