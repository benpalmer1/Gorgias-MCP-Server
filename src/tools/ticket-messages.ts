...
export function registerTicketMessageTools(server: McpServer, client: GorgiasClient) {
  ...
  server.registerTool("gorgias_list_ticket_messages", {
    ...
    inputSchema: {
      ticket_id: z.number().describe("The unique ID of the ticket whose messages to list"),
    },
  ...
  server.registerTool("gorgias_list_messages", {
    ...
    inputSchema: {
      cursor: z.string().optional().describe("Pagination cursor from a previous response's meta.next_cursor or meta.prev_cursor. Omit to start from the first page."),
      ...
      ticket_id: z.number().optional().describe("Filter messages to those belonging to a specific ticket ID"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  ...
  server.registerTool("gorgias_get_message", {
    ...
    inputSchema: {
      ticket_id: z.number().describe("The unique ID of the ticket that contains the message"),
      id: z.number().describe("The unique ID of the message to retrieve"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  ...
  server.registerTool("gorgias_create_message", {
    ...
    inputSchema: {
      ...
      integration_id: z.number().nullable().optional().describe("ID of the integration used to send the message (must be > 0)"),
      ...
      mention_ids: z.array(z.number()).nullable().optional().describe("List of User IDs to mention in an internal note. Only valid for internal-note messages."),
  ...
  server.registerTool("gorgias_update_message", {
    ...
    inputSchema: {
      // Path parameters
      ticket_id: z.number().describe("The ID of the ticket associated with the message"),
      id: z.number().describe("The ID of the message to update"),
      // Query parameter
  ...
  server.registerTool("gorgias_delete_message", {
    ...
    inputSchema: {
      ticket_id: z.number().describe("The unique ID of the ticket that contains the message to delete"),
      id: z.number().describe("The unique ID of the message to delete"),
    },
