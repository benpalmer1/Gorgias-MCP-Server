...
export function registerMacroTools(server: McpServer, client: GorgiasClient) {
  ...
  server.registerTool("gorgias_list_macros", {
    ...
    inputSchema: {
      ...
      ticket_id: idSchema.nullable().optional().describe("Order macros by the most relevant ones to reply to the given ticket. Required when order_by is 'relevance'."),
      message_id: idSchema.nullable().optional().describe("Order macros by the most relevant ones to reply to the given message. Requires order_by='relevance' and ticket_id."),
  ...
  server.registerTool("gorgias_get_macro", {
    ...
    inputSchema: {
      id: idSchema.describe("The unique ID of the macro to retrieve"),
    },
    annotations: { readOnlyHint: true, openWorldHint: true },
  ...
  server.registerTool("gorgias_update_macro", {
    ...
    inputSchema: {
      id: idSchema.describe("The unique ID of the macro to update"),
  ...
  server.registerTool("gorgias_delete_macro", {
    ...
    inputSchema: {
      id: idSchema.describe("The unique ID of the macro to delete"),
    },
  ...
  server.registerTool("gorgias_unarchive_macros", {
    ...
    inputSchema: {
      ids: z.array(idSchema).min(1).max(30).describe("List of macro IDs to unarchive (min: 1, max: 30 per request)"),
    },
    annotations: { readOnlyHint: false, idempotentHint: true, openWorldHint: true },
  }, safeHandler(async (args) => {
