...
export function projectTicket(
  ...
  return {
    ...
    assigneeName: assigneeUser?.name?.trim() ?? null,
    assigneeTeam: assigneeTeam?.name?.trim() ?? null,
    tags: tags.flatMap((t: any) => (t?.name != null ? [String(t.name).trim()] : [])),
...
export function projectMessage(message: any): ProjectedMessage {
  ...
  return {
    ...
    intents: Array.isArray(intents) ? intents.flatMap((i: any) => (i?.name != null ? [String(i.name).trim()] : [])) : [],
  };
}
